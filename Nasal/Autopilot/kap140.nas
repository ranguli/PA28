...
var locks = "/KAP140/locks";
var settings = "/KAP140/settings";
var annunciators = "/KAP140/annunciators";
...
var ptCheck = func {
	...
	if (lockPitchMode.getValue() == pitchModes["OFF"]) {
		...
		} elsif (elevatorControl.getValue() > 0.01) {# Flash the pitch trim down annunciator
...
var apButton = func {
	...
	} elsif (lockRollMode.getValue() != rollModes["OFF"] and lockPitchMode.getValue() != pitchModes["OFF"]) {
		##
...
var baroButtonTimer = func {
	...
	if (baroButtonDown == 1) {
		...
		baroButtonDown = 0;
		baroButtonPress();
	} elsif (baroButtonDown == 0 and baroSettingAdjusting == 0) {
		annunciatorBsHpaNumber.setBoolValue(0);
		annunciatorBsInhgNumber.setBoolValue(0);
		annunciatorAltNumber.setBoolValue(1);
...
var baroButtonPress = func {
	...
	if (baroButtonDown == 0 and baroTimerRunning == 0 and altButtonTimerRunning == 0) {
		baroButtonDown = 1;
		baroTimerRunning = 1;
		settimer(baroButtonTimer, 3.0);
		annunciatorAltNumber.setBoolValue(0);
...
var altAlert = func {
	...
	if (altDifference > 1000) {
		annunciatorAltAlert.getNode("state").setBoolValue(0);
	} elsif (altDifference < 1000 and altCaptured == 0) {
	...
	} elsif (altDifference < 1000 and altCaptured == 1) {
		if (altDifference > 200) {
...
var knobSmallUp = func {
	...
	} elsif (baroTimerRunning == 0 and altButtonTimerRunning == 0) {
		altCaptured = 0;
